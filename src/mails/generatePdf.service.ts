...
export class PdfService {
  ...
  async generateCombinedPDF(flightData: any, hotelData: any): Promise<Buffer> {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      doc.fontSize(20).font('Helvetica-Bold').fillColor('#005BBB').text('Voucher de Vuelo y Hotel', {
        align: 'center',
        width: doc.page.width - 2 * doc.options.margins.left,
      });
      doc.moveDown(1);
      doc.fontSize(18).font('Helvetica-Bold').fillColor('#005BBB').text('Detalle del Vuelo', {
        align: 'center',
        width: doc.page.width - 2 * doc.options.margins.left,
      });
      doc.moveDown(1);
      ...
      const destino = flightData['Destino'] || {};
      ...
      if (!destino.Llegada || !destino['Fecha de Egreso']) {
        return reject(new Error(`Missing required data for Destino section. Data received: ${JSON.stringify(destino)}`));
      }
